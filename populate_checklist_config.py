"""
Script to populate visual checklist configuration from hardcoded frontend data.
This migrates the hardcoded checklist items to the backend database.

Run with: python populate_checklist_config.py
"""
import os
import sys
import django
...
from apps.configuration.models import VisualChecklistConfig
...
def populate_checklist_config():
    """Populate visual checklist configuration from hardcoded data."""
    print("\n" + "=" * 80)
    print("POPULATE VISUAL CHECKLIST CONFIGURATION")
    print("=" * 80)
    ...
    print("\n[1] Populating LIGHT vehicle checklist...")
    ...
    print(f"  ✓ LIGHT: {created_count} created, {updated_count} updated")
    ...
    print("\n[2] Populating HEAVY vehicle checklist...")
    ...
    print(f"  ✓ HEAVY: {created_count} created, {updated_count} updated")
    heavy_total = created_count + updated_count
    
    print("\n" + "=" * 80)
    print("SUMMARY")
    print("=" * 80)
    print(f"  LIGHT vehicle items: {light_total}")
    print(f"  HEAVY vehicle items: {heavy_total}")
    print(f"  Total items: {light_total + heavy_total}")
    print("\n✓ Checklist configuration populated successfully!")
    print("  Items are now manageable via Django admin or API.")
    print("  Frontend will fetch from backend instead of using hardcoded data.")
    print("\n" + "=" * 80 + "\n")

if __name__ == '__main__':
    populate_checklist_config()
